...
public class PermissionManager extends Fragment implements Runnable {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        boolean isInstallEnable = (permissions.contains(PermissionCode.REQUEST_INSTALL_PACKAGES)
                && !PermissionTool.isHasInstallPermission(getActivity()));
        boolean isAlertEnable = (permissions.contains(PermissionCode.SYSTEM_ALERT_WINDOW)
                && !PermissionTool.isHasOverlaysPermission(getActivity()));
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        for (int i = 0;i < permissions.length; i++){
            ...
            if (PermissionCode.REQUEST_INSTALL_PACKAGES.equals(permissions[i])) {
                if (PermissionTool.isHasInstallPermission(getActivity())) {
            ...
            if (PermissionCode.SYSTEM_ALERT_WINDOW.equals(permissions[i])) {
                if (PermissionTool.isHasOverlaysPermission(getActivity())) {
                    grantResults[i] = PackageManager.PERMISSION_GRANTED;
                } else {
                    grantResults[i] = PackageManager.PERMISSION_DENIED;
                }
            ...
            if (permissions[i].equals(PermissionCode.ANSWER_PHONE_CALLS)
                ...
                //检查当前的安卓版本是否符合要求
                if (!PermissionTool.isOverO()) {
                    grantResults[i] = PackageManager.PERMISSION_GRANTED;
                }
            }
        }

        List<String> successPermissions = PermissionTool.getSuccessPermissions(permissions, grantResults);
        if (successPermissions.size() == permissions.length){
            listener.hasPermission(successPermissions, true);
        } else {
            List<String> failPermissions = PermissionTool.getFailPermissions(permissions, grantResults);
            if (getArguments().getBoolean(PERMISSION_CONNECT)
                    && PermissionTool.isRequestDeniedPermission(getActivity(), failPermissions)){
                requestPermission();
                return;
            }

            listener.noPermission(failPermissions,
                    PermissionTool.checkMorePermanentDenied(getActivity(), failPermissions));
